 ...
 */
public class db9_1 extends HttpServlet {
    ...
    out.println("<html>");
    out.println("<head><title>ブラックリスト</title></head>");
    out.println("<body>");
           
    request.setCharacterEncoding("UTF-8");
    
    String prof =request.getParameter("profileID");
    ...
    //out.println(prof +","+ name + ","+ tel + "," + age + "," + brt);
    
    Class.forName("com.mysql.jdbc.Driver").newInstance();
    
    c = DriverManager.getConnection("jdbc:mysql://localhost:8889/challenge_db","root","root");
    s =c.prepareStatement("INSERT INTO profiles (profileID, name, tel, age, birthday) VALUES (?, ?, ?, ?, ?);");
    // 代入する値は?にしておく。
    
    //s =c.prepareStatement("INSERT INTO profiles (profileID,name,tel,age,birthday)"
    //        + "VALUES("+prof+","+name+","+tel+","+age+","+brt+")");
    
    s.setInt(1, Integer.parseInt(prof));
    s.setString(2, name);
    s.setString(3, tel);
    s.setInt(4, Integer.parseInt(age));
    s.setString(5, brt);
    //代入する値を、intやStringで改めて定義。
